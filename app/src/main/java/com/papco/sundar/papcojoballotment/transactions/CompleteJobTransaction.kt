...
class CompleteJobTransaction(jobToRemove: PrintJob, machineId: String) :
    ...
    override fun apply(transaction: Transaction) {
        ...
        if (jobDocument.exists()) {
            ...
            job?.completedOn = Calendar.getInstance(Locale.getDefault()).timeInMillis
        } else
        ...
        if (machineDocument.exists()) {
            ...
            machine?.let {
                ...
                it.lastCompletion=job!!.completedOn
            }
        } else
            throw FirebaseFirestoreException(
                "Machine not found",
